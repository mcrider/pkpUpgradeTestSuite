 ...
 *
 * $Id: individualSubscriptions.tpl,v 1.5 2009/10/14 22:16:36 asmecher Exp $
 *}
{strip}
...
{iterate from=subscriptions item=subscription}
	...
	<tr valign="top">
		...
		<td><a href="{url op="editSubscription" path="individual"|to_array:$subscription->getSubscriptionId()}" class="action">{translate key="common.edit"}</a>{if !$isNonExpiring}&nbsp;|&nbsp;<a href="{url op="renewSubscription" path="individual"|to_array:$subscription->getSubscriptionId()}" class="action">{translate key="manager.subscriptions.renew"}</a>{/if}&nbsp;|&nbsp;<a href="{url op="deleteSubscription" path="individual"|to_array:$subscription->getSubscriptionId()}" onclick="return confirm('{translate|escape:"jsparam" key="manager.subscriptions.confirmDelete"}')" class="action">{translate key="common.delete"}</a></td>
	</tr>
	<tr>
