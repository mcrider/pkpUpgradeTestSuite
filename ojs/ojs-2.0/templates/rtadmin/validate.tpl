 ...
 *
 * $Id: validate.tpl,v 1.6 2005/05/15 10:27:42 kevin Exp $
 *}
...
{iterate from=versions item=version}
	...
	{foreach from=$version->getContexts() item=context}
		...
		{foreach from=$errors item=error}
			<br />
			{translate key="rt.admin.validateUrls.urlIsInvalid" url=$error.url|truncate:50|escape}&nbsp;&nbsp;<a href="{$requestPageUrl}/editSearch/{$version->getVersionId()}/{$context->getContextId()}/{$error.id}" class="action">{translate key="common.edit"}</a>
		{foreachelse}
